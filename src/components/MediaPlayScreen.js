...
export default (props) =>{
  ...
  function msToTime(ms) {
    ...
    var s = ("0" + Math.floor(((ms/1000/60/60 - h)*60 - m)*60)).slice(-2);
      ...
      <View>
        ...
        <View style={{alignItems: 'center', marginVertical: 20}}>
          <View style={{flexDirection: 'row', width: 250, justifyContent: 'space-between', alignItems: 'center'}}>
              <Button transparent disabled={!isLoaded} style={styles.controlButton} onPress={playPrevious}>
                <Ionicons style={[!isLoaded && styles.disabled, styles.controlIcons]} size={34} name="ios-skip-backward" />
              </Button>
              <Button transparent onPress = {onPlayPausePressed} style={{height: 70}} disabled={!isLoaded}>
                <MaterialIcons style={[!isLoaded && styles.disabled, styles.controlIcons]} size={60} name={isPlaying ? 'pause-circle-filled' : 'play-circle-filled'} />
              </Button>
              <Button transparent disabled={!isLoaded} style={styles.controlButton} onPress={playNext}>
                <Ionicons style={[!isLoaded && styles.disabled, styles.controlIcons]} size={34} name="ios-skip-forward" />
              </Button>
          </View>
        </View>
        ...
        <View>
          ...
          <View style={{justifyContent: 'space-between', flexDirection: 'row'}}>
            <Text style={styles.soundStamp}>{msToTime(audioPosition) || '0.00'}</Text>
            <Text style={styles.soundStamp}>- {msToTime(audioDuration) || '0.00'}</Text>
          </View>
          {
            isBuffering && (
              <Text style={{textAlign: 'center', color: PRIMARY_FONT_COLOR}}> Loading...</Text>
            )
          }
        </View>
      </View>
...
const styles = StyleSheet.create({
  ...
  controlButton: {
    height: 42, width: 42, borderRadius: 42/2,
